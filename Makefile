...
ci-job-normal: assert-docker
	...
	pytest --cov=metarl --cov-report=xml -m \
...
ci-job-large: assert-docker
	[ ! -f $(MJKEY_PATH) ] || mv $(MJKEY_PATH) $(MJKEY_PATH).bak
	pytest --cov=metarl --cov-report=xml -m 'large and not flaky' --durations=20
	bash <(curl -s https://codecov.io/bash)

ci-job-mujoco: assert-docker
	pytest --cov=metarl --cov-report=xml -m 'mujoco and not flaky' --durations=20
	bash <(curl -s https://codecov.io/bash)

ci-job-mujoco-long: assert-docker
	pytest --cov=metarl --cov-report=xml -m 'mujoco_long and not flaky' --durations=20
	bash <(curl -s https://codecov.io/bash)
...
ci-job-verify-envs-conda: METARL_BIN = $(CONDA_ROOT)/envs/metarl-ci/bin
ci-job-verify-envs-conda:
	...
	$(CONDA) info -a
	$(CONDA) create -n metarl-ci python=3.5 pip -y
	$(METARL_BIN)/pip install --upgrade pip
	$(METARL_BIN)/pip install dist/metarl.tar.gz[all]
	$(METARL_BIN)/pip install dist/metarl.tar.gz[all,dev]
	# pylint will verify all imports work
	$(METARL_BIN)/pylint --disable=all --enable=import-error metarl
...
ci-job-verify-envs-pipenv:
	...
	pip install pipenv
	pipenv --python=3.5
	pipenv install dist/metarl.tar.gz[all,dev]
	pipenv graph
	# pylint will verify all imports work
	pipenv run pylint --disable=all --enable=import-error metarl

ci-deploy-docker: assert-travis
	echo "${DOCKER_API_KEY}" | docker login -u "${DOCKER_USERNAME}" \
		--password-stdin
	docker tag "${TAG}" rlworkgroup/metarl-ci:latest
	docker push rlworkgroup/metarl-ci

build-ci: TAG ?= rlworkgroup/metarl-ci:latest
...
build-headless: TAG ?= rlworkgroup/metarl-headless:latest
...
build-nvidia: TAG ?= rlworkgroup/metarl-nvidia:latest
...
run-ci: TAG ?= rlworkgroup/metarl-ci
run-ci:
	docker run \
		-e TRAVIS_BRANCH \
...
run-headless: CONTAINER_NAME ?= metarl-headless
run-headless: build-headless
	docker run \
		-it \
		--rm \
		-v $(DATA_PATH)/$(CONTAINER_NAME):/root/code/metarl/data \
		-e MJKEY="$$(cat $(MJKEY_PATH))" \
		--name $(CONTAINER_NAME) \
		${RUN_ARGS} \
		rlworkgroup/metarl-headless ${RUN_CMD}

run-nvidia: ## Run the Docker container for machines with NVIDIA GPUs
run-nvidia: CONTAINER_NAME ?= metarl-nvidia
run-nvidia: build-nvidia
	...
	docker run \
		...
		--name $(CONTAINER_NAME) \
		${RUN_ARGS} \
		rlworkgroup/metarl-nvidia ${RUN_CMD}
