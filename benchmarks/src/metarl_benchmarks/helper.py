...
Example:
    @benchmark
    def ppo_benchmarks():
        iterate_experiments(ppo_metarl_tf)
        iterate_experiments(ppo_metarl_pytorch)

    @benchmark(plot=False)
    def my_benchmarks():
        iterate_experiments(trpo_metarl_pytorch, env_ids=env_ids)
        iterate_experiments(vpg_metarl_pytorch, seeds=seeds)
...
_bucket = storage.Client().bucket('resl-metarl-benchmarks')
