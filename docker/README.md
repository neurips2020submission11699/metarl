...
```
docker run -it --rm rlworkgroup/metarl-headless python examples/tf/trpo_cartpole.py
```
...
docker run \
  ...
  rlworkgroup/metarl-headless python examples/tf/trpo_swimmer.py
```
...
docker run \
  ...
  rlworkgroup/metarl-headless python examples/tf/trpo_cartpole.py
```
...
image, except that the image name is defined by `rlworkgroup/metarl-nvidia`.
For example, to execute a launcher file:
```
docker run -it --rm rlworkgroup/metarl-nvidia python examples/tf/trpo_cartpole.py
```
