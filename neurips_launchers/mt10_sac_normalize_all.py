...
def mt10_sac_normalize_all(ctxt=None, seed=1):
    """Set up environment and algorithm and run the task."""
    runner = LocalRunner(ctxt)
    MT10_envs_by_id = {}
    MT10_envs_test = {}
    for (task, env) in EASY_MODE_CLS_DICT.items():
        MT10_envs_by_id[task] = MetaRLEnv(normalize(env(*EASY_MODE_ARGS_KWARGS[task]['args'],
                                                        **EASY_MODE_ARGS_KWARGS[task]['kwargs']),
                                                    normalize_obs=True,
                                                    flatten_obs=False,
                                                    normalize_reward=True))
        # python 3.6 dicts are ordered
        MT10_envs_test[task] = MetaRLEnv(normalize(env(*EASY_MODE_ARGS_KWARGS[task]['args'],
                                                       **EASY_MODE_ARGS_KWARGS[task]['kwargs']),
                                                    normalize_obs=True,
                                                    flatten_obs=False,
...
mt10_sac_normalize_all(seed=532)
