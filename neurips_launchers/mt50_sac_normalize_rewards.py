...
def mt50_sac(ctxt=None, seed=1):
    ...
    MT50_envs_by_id = {name: MetaRLEnv(normalize_reward(env)) for (name,env) in zip (envs._task_names, envs._task_envs)}
    MT50_envs_test = {name: MetaRLEnv(env) for (name,env) in zip (test_envs._task_names, test_envs._task_envs)}
