#!/usr/bin/env bash
# This script installs metarl on Linux distributions.
#
...
print_help ()
{
  ...
  printf '%s\n' "required by metarl in .bashrc (off by default)"
  printf '\t%s\n' "-h,--help: Prints help"
}
...
METARL_DIR="$(readlink -f $(dirname $0)/..)"
cd "${METARL_DIR}"
...
# For installing metarl: wget, bzip2, unzip, git
...
if [[ "${_arg_modify_bashrc}" = on ]]; then
  echo -e "\n# Added by the metarl installer" >> "${BASH_RC}"
fi
