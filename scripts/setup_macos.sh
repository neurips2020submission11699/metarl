#!/usr/bin/env bash
# This script installs metarl on macOS distributions.
#
...
# Add macOS versions where metarl is successfully installed in this list
...
if [[ ! " ${VERIFIED_MACOS_VERSIONS[@]} " =~ " ${VER} " ]]; then
  print_warning "You are attempting to install metarl on a version of macOS" \
    "which we have not verified is working." | fold -s
  print_warning "\nmetarl relies on community contributions to support macOS\n"
  print_warning "If this installation is successful, please add your macOS" \
    "version to VERIFIED_MACOS_VERSIONS to" \
    "https://github.com/rlworkgroup/metarl/blob/master/scripts/setup_macos.sh" \
    "on GitHub and submit a pull request to rlworkgroup/metarl to help out" \
...
dir_err_txt="Please run this script only from the root of the metarl \
repository, i.e. you should run it using the command \
\"bash scripts/setup_macos.sh\""
if [[ ! -f setup.py ]] && [[ ! $(grep -Fq "name='rlmetarl'," setup.py) ]]; then
...
# Make sure that we're under the metarl directory
METARL_DIR="$(dirname $(script_dir_path))"
cd "${METARL_DIR}"
...
echo "Installing metarl dependencies"
...
# For installing metarl: bzip2, git, glfw, unzip, wget
...
if [[ "${_arg_modify_shellrc}" = on ]]; then
  echo -e "\n# Added by the metarl installer" >> "${BASH_PROF}"
  echo -e "\n# Added by the metarl installer" >> "${ZSHRC}"
fi
...
# Add metarl to python modules
if [[ "${_arg_modify_shellrc}" != on ]]; then
  echo -e "\nRemember to execute the following commands before running metarl:"
  echo "${LD_LIB_ENV_VAR}"
  echo "You may wish to edit your .bash_profile to prepend these commands."
fi

echo -e "\nmetarl pre-requisites are installed! To make the changes take " \
