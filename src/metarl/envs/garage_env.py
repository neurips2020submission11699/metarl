"""Wrapper class that converts gym.Env into MetaRLEnv."""
...
class MetaRLEnv(gym.Wrapper):
    """Returns an abstract MetaRL wrapper class for gym.Env.

    In order to provide pickling (serialization) and parameterization
    for gym.Envs, they must be wrapped with a MetaRLEnv. This ensures
    compatibility with existing samplers and checkpointing when the
    envs are passed internally around metarl.

    Furthermore, classes inheriting from MetaRLEnv should silently
    ...
    def spec(self):
        ...
        Returns:
            metarl.envs.env_spec.EnvSpec: The envionrment specification.
    ...
    def step(self, action):
        ...
        # 'MetaRLEnv.TimeLimitTerminated'
        if 'TimeLimit.truncated' in info:
            info['MetaRLEnv.TimeLimitTerminated'] = done  # done = True always
