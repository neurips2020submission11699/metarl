"""Tools for running experiments with MetaRL."""
import base64
import collections
import datetime
import enum
...
def run_experiment(method_call=None,
                   ...
                   exp_name=None,
                   log_dir=None,
                   script='metarl.experiment.experiment_wrapper',
    ...
    for task in batch_tasks:
        ...
        task['env']['METARL_FORCE_CPU'] = str(force_cpu)
...
def to_local_command(
        params,
        python_command='python',
        script='metarl.experiment.experiment_wrapper'):  # noqa: D103,E501
    ...
    metarl_env = eval(os.environ.get('METARL_ENV', '{}'))
    for k, v in metarl_env.items():
...
class ExperimentTemplate:
    ...
    This class should only be created by calling metarl.wrap_experiment.
