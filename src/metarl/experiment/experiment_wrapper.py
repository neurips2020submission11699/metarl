...
import metarl.experiment
import metarl.plotter
import metarl.tf.plotter
...
def child_proc_shutdown(children):
    ...
    Args:
        children (list[metarl.plotter.Plotter]): Instances of plotter to
            shutdown.
    ...
    if alive:
        ...
        error_msg += ('https://github.com/rlworkgroup/metarl/issues/120')
        print(error_msg)
...
def log_parameters(log_file, args):
    ...
    with open(log_file, 'w') as f:
        json.dump(log_params,
                  ...
                  indent=2,
                  sort_keys=True,
                  cls=metarl.experiment.experiment.LogEncoder)
    metarl.experiment.experiment.dump_json(log_file, log_params)
