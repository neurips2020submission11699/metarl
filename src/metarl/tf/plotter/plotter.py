...
from metarl.sampler.utils import rollout as default_rollout
...
class Plotter:
    ...
    Args:
        ...
        crashes).
        policy (metarl.tf.Policy): The policy to do the rollouts with.
    ...
    @staticmethod
    def get_plotters():
        """Return all metarl.tf.Plotter's.

        Returns:
            list[metarl.tf.Plotter]: All the metarl.tf.Plotter's
    ...
    def update_plot(self, policy, max_length=np.inf):
        ...
        Args:
            policy (metarl.tf.Policy): The policy to rollout.
