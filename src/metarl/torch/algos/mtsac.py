...
from metarl.torch import global_device
...
class MTSAC(SAC):
    ...
    Args:
        policy (metarl.torch.policy.Policy): Policy/Actor/Agent that is being
            optimized by SAC.
        qf1 (metarl.torch.q_function.ContinuousMLPQFunction): QFunction/Critic
            used for actor/policy optimization. See Soft Actor-Critic and
            Applications.
        qf2 (metarl.torch.q_function.ContinuousMLPQFunction): QFunction/Critic
            used for actor/policy optimization. See Soft Actor-Critic and
            Applications.
        replay_buffer (metarl.replay_buffer.ReplayBuffer): Stores transitions
            that are previously collected by the sampler.
        env_spec (metarl.envs.env_spec.EnvSpec): The env_spec attribute of the
        ...
        eval_env (metarl.envs.MetaRLEnv): The environment used for collecting
